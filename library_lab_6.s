...
delay:
	...
	LDR r2, [r0, #0x4]		; Put timer into periodic mode
	ORR r2, r2, #0x2
	STR r2, [r0, #0x4]
	...
	LDR r2, [r0, #0x18]		; Set timer to interrupt when top limit of timer is reached
	ORR r2, r2, #0x1
	STR r2, [r0, #0x18]

; ENABLE Timer Interrupts by enabling NVIC (Nested Vector Interrupt Controller) by writing a 1 to bit 19 of EN0
	MOV r1, #0xE100
	MOVT r1, #0xE000
	LDR r2, [r1]
	MOV r3, #0x0000
	...
	LDR r2, [r0, #0xC]		; enable the timer
	ORR r2, r2, #0x1
	STR r2, [r0, #0xC]
...
thousandascii:
	CMP r9, #1000		; Compares r9 to 1000
	BLT hundredascii	; If less than, branch to hundred
	ADD r1, r1, #1		; Add 1 to r4 (running total for thousands place)
	SUB r9, r9, #1000	; Subtract 1000 from r9
	B thousand			; Branch to thousand

hundredascii:
	CMP r9, #100		; Compares r9 to 100
	BLT tensascii		; If less than, branch to tens
	ADD r2, r2, #1		; Add 1 to r1 (running total for hundreds place)
	SUB r9, r9, #100	; Subract 100 from r9
	B hundredascii		; Branch to hundred

tensascii:
	CMP r9, #10			; Compares r9 to 10
	BLT onesascii		; If less than, branch to ones
	ADD r3, r3, #1		; Add 1 to r2 (running total for tens place)
	SUB r9, r9, #10		; Subtract 10 from r9
	B tensascii			; Branch to tens

onesascii:
	CMP r9, #1			; Compares r9 to 1
	BLT printascii		; If less than, branch to print1
	ADD r4, r4, #1		; Add 1 to r3 (running total for ones place)
	SUB r9, r9, #1		; Subtract 1 from r9
	B onesascii			; Branch to ones
